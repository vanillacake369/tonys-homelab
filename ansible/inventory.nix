...
let
  ...
  jumpHost = if deploy_target != "" then deploy_target else host.deployment.targetHost;
  jumpUser = host.deployment.targetUser;
  
  # Identity file logic
  idFile = builtins.getEnv "SSH_KEY_FILE";
  idArg = if idFile != "" then "-o IdentityFile=${idFile}" else "";
  ...
  in {
    ...
    node_vlan     = vm.vlan;
    node_role     = if builtins.elem name k8s.masters then "master" else "worker";
  };
