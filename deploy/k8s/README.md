...
GitRepository (source-controller가 주기적 poll)
  │
  ▼
Kustomization/flux-system (deploy/k8s/clusters/homelab/ 적용)
  │
  ├──▶ Kustomization/infrastructure (deploy/k8s/infrastructure/ 적용)
  │       └── HelmRepository 등록 (Bitnami 등)
  │
  └──▶ Kustomization/apps (deploy/k8s/apps/ 적용, infrastructure 완료 후)
...
│       │   └── kustomization.yaml
│       ├── infrastructure.yaml     # → deploy/k8s/infrastructure/ 를 reconcile
│       └── apps.yaml               # → deploy/k8s/apps/ 를 reconcile (infrastructure 이후)
│
...
```yaml
# deploy/k8s/infrastructure/sources/my-repo.yaml
---
...
```bash
mkdir -p deploy/k8s/apps/my-app
```

3. **HelmRelease 작성:**

```yaml
# deploy/k8s/apps/my-app/helmrelease.yaml
---
...
```yaml
# deploy/k8s/apps/my-app/kustomization.yaml
---
...
```yaml
# deploy/k8s/infrastructure/sources/external-repo.yaml
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: GitRepository
metadata:
